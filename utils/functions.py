...
def read_single_instance_with_gaz(lines, gaz, word_alphabet, biword_alphabet, char_alphabet, gaz_alphabet, label_alphabet, number_normalized, max_sent_length, char_padding_size=-1, char_padding_symbol = '</pad>'):
    #in_lines = open(input_file,'r', encoding='utf-8').readlines()
    in_lines = lines
    instence_texts = []
    instence_Ids = []
    ...
    for idx in range(len(in_lines)):
        ...
        if len(line) > 2:
            ...
            gaz_Ids = []
    print('='*23)
    print(instence_Ids, instence_texts)
    return instence_texts, instence_Ids
